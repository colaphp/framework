...
class Validator
{
    ...
    public function check($data, $rules = [], $scene = '')
    {
        ...
        foreach ($rules as $key => $rule) {
            ...
            } elseif ($rule instanceof ValidRule) {
                //  验证因子
    ...
    public function checkRule($value, $rules)
    {
        ...
        } elseif ($rules instanceof ValidRule) {
            $rules = $rules->getRule();
    ...
    protected function checkItem($field, $value, $rules, $data, $title = '', $msg = [])
    {
        ...
        foreach ($rules as $key => $rule) {
            if ($rule instanceof \Closure) {
                ...
                if ($info == 'must' || strpos($info, 'require') === 0 || (! is_null($value) && $value !== '')) {
                    // 验证类型
    ...
    public function is($value, $rule, $data = [])
    {
        ...
        switch (lcfirst($rule)) {
            ...
            case 'file':
                ...
                break;
            case 'image':
                $result = $value instanceof SplFileInfo && in_array($this->getImageType($value->getRealPath()), [1, 2, 3, 6]);
                break;
            default:
    ...
    public function image($file, $rule)
    {
        ...
        if ($rule) {
            ...
            if (isset($rule[2])) {
                ...
                if ($imageType != image_type_to_extension($type, false)) {
                    return false;
                }
            }
    ...
    {
        return $_SERVER['REQUEST_METHOD'] == strtoupper($rule);
    }
    ...
    public function requireIf($value, $rule, $data)
    {
        ...
        if ($val == $this->getDataValue($data, $field)) {
            return ! empty($value) || $value == '0';
        } else {
            return true;
        }
    ...
    {
        return (new Captcha())->check($data['uuid'], $value);
    }
    ...
    protected function getRuleMsg($attribute, $title, $type, $rule)
    {
        ...
        } elseif (strpos($type, 'require') === 0) {
            $msg = self::$typeMsg['require'];
        } else {
            $msg = $title.'规则不符';
        }

        if (is_string($msg) && is_scalar($rule) && strpos($msg, ':') !== false) {
            // 变量替换
            if (is_string($rule) && strpos($rule, ',')) {
