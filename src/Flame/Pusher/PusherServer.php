...
class PusherServer
{
     ...
     * api监听的ip端口
     */
    public string $apiListen = 'http://0.0.0.0:1080';

    /**
     * webhook 延迟设置
     */
    public int $webHookDelay = 3;

    /**
     * 全局数据快照
     */
    protected array $_globalDataSnapshot = [];

    /**
     * 事件对应的客户端链接
     */
    protected array $_eventClients = [];

    /**
     * 所有的客户端链接
     */
    protected array $_allClients = [];
     ...
     * )
     */
    protected array $_globalData = [];

    /**
     * 当前进程全局唯一订阅id
     */
    protected string $_globalID = '1';

    /**
     * 构造函数
     */
    public function __construct(string $api_listen, array $app_info)
    {
    ...
    public function onWorkerStart($worker): void
    {
        ...
        $api_worker->onMessage = [$this, 'onApiClientMessage'];
        $api_worker->listen();
     ...
     * 当websocket握手时
     */
    public function onWebSocketConnect(TcpConnection $connection, $header): void
    {
        ...
        switch ($event) {
            ...
            default:
                if (str_starts_with($event, 'pusher:')) {
                    $connection->send($this->error(null, 'Unknown event'));
                ...
                if ($channel_type !== 'private' && $channel_type !== 'presence') {
                    ...
                    $connection->send($this->error(null, 'Client event rejected - only supported on private and presence channels'));

                    return;
                }
                // 当前链接没有订阅这个channel
                if (! isset($connection->channels[$channel])) {
                    $connection->send($this->error(null, 'Client event rejected - you didn\'t subscribe this channel'));

                    return;
                }
                // 事件必须以client-为前缀
                if (! str_starts_with($event, 'client-')) {
                    $connection->send($this->error(null, 'Client event rejected - client events must be prefixed by \'client-\''));
     ...
     * 组装失败信息
     */
    protected function error($code, $message): string
    {
     ...
     * 客户端订阅channel
     */
    public function subscribePublicChannel($connection, $channel): void
    {
        $app_key = $connection->appKey;
        ...
        if (! isset($this->_globalData[$app_key][$channel])) {
            $this->_globalData[$app_key][$channel] = [
                'type' => 'presence',
     ...
     * 客户端订阅channel
     */
    public function subscribePrivateChannel($connection, $channel): void
    {
        $this->subscribePublicChannel($connection, $channel);
    }
     ...
     * 客户端订阅channel
     */
    public function subscribePresence($connection, $channel, $uid, $user_info): void
    {
        $app_key = $connection->appKey;
        ...
        if (! isset($this->_globalData[$app_key][$channel]['users'][$uid]['user_info'])) {
            $this->_globalData[$app_key][$channel]['users'][$uid] = ['user_info' => $user_info, 'ref_count' => 0];
            $member_added = true;
        }
        ...
        if ($member_added) {
            ...
            $this->publishToClients($app_key, $channel, 'pusher_internal:member_added', json_encode([
                'user_id' => $uid,
                'user_info' => $user_info,
    ...
    public function getPresenceChannelDataForSubscribe($app_key, $channel): array
    {
        $hash = [];
        $count = 100;
        if (isset($this->_globalData[$app_key][$channel])) {
     ...
     * 客户端取消订阅channel
     */
    public function unsubscribePublicChannel($connection, $channel): void
    {
        $app_key = $connection->appKey;
     ...
     * 客户端取消订阅channel
     */
    public function unsubscribePrivateChannel($connection, $channel): void
    {
        $this->unsubscribePublicChannel($connection, $channel);
    }
     ...
     * 客户端取消订阅channel
     */
    public function unsubscribePresenceChannel($connection, $channel, $uid): void
    {
        $app_key = $connection->appKey;
        ...
        if ($member_removed) {
            ...
            $this->publishToClients($app_key, $channel, 'pusher_internal:member_removed', json_encode(['user_id' => $uid], JSON_UNESCAPED_UNICODE));
        }
        unset($connection->channels[$channel], $this->_eventClients[$connection->appKey][$channel][$connection->socketID]);
    }
     ...
     * 发布事件
     */
    public function publishToClients($app_key, $channel, $event, $data, $socket_id = null): void
    {
            ...
            case 'channels':
                ...
                if (isset($explode[4])) {
                    ...
                    $id_array = isset($this->_globalData[$app_key][$channel]['users']) ?
                        array_keys($this->_globalData[$app_key][$channel]['users']) : [];
                    $user_id_array = [];
                ...
                return $connection->send(json_encode($channel_info, JSON_UNESCAPED_UNICODE));
            default:
                return $connection->send(new Response(400, [], 'Bad Request'));
        }
    }
    ...
    public function webHookCheck(): void
    {
        ...
        $this->webHookSend(['channel_events' => $channel_events, 'user_events' => $user_events]);
    }
    ...
    protected function sendHttpRequest($address, $app_key, $secret, $body, $redirect_count = 0)
    {
        ...
        Timer::add(10, [$client, 'close'], null, false);
        $client->connect();
    }
